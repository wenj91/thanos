...
func runRule(
	...
	if len(confContentYaml) > 0 {
		...
		s := shipper.New(logger, reg, conf.dataDir, "fake", bkt, func() labels.Labels { return conf.lset }, metadata.RulerSource, false, conf.shipper.allowOutOfOrderUpload, metadata.HashFunc(conf.shipper.hashFunc))
